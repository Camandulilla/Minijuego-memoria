...
const crearColeccionDeCartasInicial = (
  ...
  /* Aquí crearemos un array de cartas a partir de un array de infoCartas
    ...
    */
  const arrayDuplicado: Carta[] = [];

  for (const item of infoCartas) {
    for (let index = 0; index < repeticiones; index++) {
      arrayDuplicado.push(crearCartaInicial(item.idFoto, item.imagen));
    }
  }
...
export let cartas: Carta[] = crearColeccionDeCartasInicial(infoCartas, 2);
...
export interface Tablero {
  ...
  indiceCartaVolteadaA?: number | null;
  indiceCartaVolteadaB?: number | null;
}
